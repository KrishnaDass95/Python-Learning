...
class Item:
    ...
    def instantiateFromCSV(cls):  # we aren't passing self cause we're not creating any objects that get passed
        with open('items.csv', 'r') as f: # context manager to read the file, open items.csv in read mode
            reader = csv.DictReader(f) # convert the csv into a python dictonary
            items = list(reader) # put that/ convert the dict into a list
        for item in items: # objects initialise
            Item(
                name=item.get('name'),
    ...
    def is_integer(num): # static method is like isolated function
        # we will remove the floats that have point 0
        # For i.e: 5.0 -> 5,
